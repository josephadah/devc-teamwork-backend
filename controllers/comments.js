...
const getAllArticles = (req, res) => {
  try {
    pool.query(`SELECT * FROM comments ORDER BY "createdOn" DESC`, (error, results) => {
      if (error) {
        throw error;
      }
...
const getArticle = (req, res) => {
  try {
    ...
    pool.query(`SELECT * FROM comments WHERE "articleId" = $1`, [id],
      (error, results) => {
...
const postArticle = (req, res) => {
  try {
    ...
    pool.query(
      `INSERT INTO comments (title, article, "userId") VALUES ($1, $2, $3) RETURNING *`,
  ...
  try {
    ...
    pool.query(`SELECT * FROM comments WHERE "articleId" = $1`, [id],
      (error, results) => {
        ...
        pool.query(
          `UPDATE comments SET title = $1, article = $2 WHERE "articleId" = $3`,
          [title, article, id],
          (error, result) => {
            if (error) throw error;
            res.status(200).json({
...
const deleteArticle = (req, res) => {
  try {
    ...
    pool.query(`SELECT * FROM comments WHERE "articleId" = $1`, [id], (error, results) => {
      if (error) throw error;
      ...
      if (article) {
        ...
        pool.query(`DELETE FROM comments WHERE "articleId" = $1`, [id], (error, result) => {
            if (error) throw error;
            res.status(200).json({
              status: "success",
              data: {
